...
def matchDicts(dict1, dict2):
    ...
    for c in dict1:
        ...
        if c in dict2 and dict1[c] == dict2[c]: matches += 1
...
def modifyDict(sDict, wDict, symbol, countModifier):
    ...
    if symbol not in sDict: sDict[symbol] = 0

    if symbol in wDict and sDict[symbol] == wDict[symbol]: ans = -1
...
occurrences = 0
if matchingLetters == len(wDict): occurrences += 1
